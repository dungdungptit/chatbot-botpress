...
const printModulesConfig = async (botId?: string) => {
  ...
  await Promise.mapSeries(configs, async file => {
    printSub(`${botId ? `Bot ${botId}` : 'Tổng quan'} - Mô-đun ${path.basename(file, '.json')}`)
    printObject(await ghost.readFileAsObject<any>('./config', file), includePasswords)
  })
}

const printGeneralInfos = async () => {
  printHeader('Tổng quan')
  printRow('Phiên bản Botpress', process.BOTPRESS_VERSION)
  printRow('Phiên bản NodeJS', process.version.substr(1))
  printRow('Phiên bản NLU', await getToolVersion('nlu'))
  printRow('Phiên bản Studio', await getToolVersion('studio'))
  printRow('Phiên bản của tin nhắn', await getToolVersion('messaging'))
  printRow('File nhị phân đang chạy', process.pkg ? 'Có' : 'Không')
  printRow('Doanh nghiệp', process.IS_PRO_AVAILABLE ? (process.IS_PRO_ENABLED ? 'Đã kích hoạt' : 'Khả dụng') : 'Không khả dụng')
  printRow('Hostname', os.hostname())
  printRow(
    'Thông tin host',
    `${process.distro} với ${os.cpus().length} CPUs và ${_.round(os.totalmem() / (1024 * 1024 * 1024))} GB RAM`
  )

  testWriteAccess('Vị trí thực thi', process.PROJECT_LOCATION)
  testWriteAccess('Thư mục dữu liệu', path.resolve(process.PROJECT_LOCATION, 'data'))
  testWriteAccess('Thư mục dữ liệu ứng dụng', process.APP_DATA_PATH)
}

const testConnectivity = async () => {
  printHeader('Kết nối')

  await wrapMethodCall('Đang kết nối đến Cơ sử dữ liệu', async () => {
    await app.database.initialize()
    printRow('Loại cơ sở dữ liệu', app.database.knex.isLite ? 'SQLite' : 'Postgres')
  ...
  await wrapMethodCall('Đang kết nối đến Redis', async () => {
  ...
  await wrapMethodCall('Kiếm tra cơ bản Redis', async () => {
  ...
  try {
    ...
    process.env.BP_REDIS_SCOPE && printRow('Redis đang sử dụng phạm vi', process.env.BP_REDIS_SCOPE)
    printRow('Các nút Botpress đang lắng nghe trên Redis', reply[1])
  } catch (err) {}

  try {
    for (const client of await getClientsList(redisClient)) {
      printRow(`- Client ${client.parsed.name}`, `Thời gian hoạt động: ${client.parsed.age}s`)
    }
  } catch (err) {
    printRow('- Gặp lỗi khi nhận danh sách khách hàng', err)
  }

  try {
    // @ts-ignore typing missing for that method
    const reply = await redisClient.pubsub(['NUMSUB', makeRedisKey('cache_status')])
    printRow('Các nút nhắn tin đang lắng nghe trên Redis', reply[1])
  } catch (err) {}
}

const testNetworkConnections = async () => {
  printHeader('Kết nối mạng')

  const hosts = [
    {
      label: 'Google (truy cập bên ngoài)',
      url: 'https://google.com'
    },
    {
      label: 'Máy chủ đang được cấp phép',
  ...
  try {
    ...
    if (duckling) {
      hosts.push({ label: 'Máy chủ Duckling', url: duckling })
    }

    if (langServer) {
      hosts.push({ label: 'Máy chủ ngôn ngữ', url: `${langServer}/languages` })
    }
  } catch (err) {}
...
const testServiceConnections = async () => {
  ...
  printHeader('Kết nối tới Dịch vụ')
...
const printDatabaseTables = async () => {
  printHeader('Các bảng cơ sở dữ liệu')
...
const listEnvironmentVariables = () => {
  printHeader('các biến môi trường')
...
const printConfig = async () => {
  printHeader('Cấu hình Botpress')
  printObject(botpressConfig, includePasswords)

  printHeader('Cấu hình mô-đun')
...
const printMigrationHistory = async () => {
  ...
  printHeader('Lịch sử Migration')
  history.forEach(entry => {
    print(`Migration từ ${entry.initialVersion} đến ${entry.targetVersion} (${moment(entry.created_at)})`)
