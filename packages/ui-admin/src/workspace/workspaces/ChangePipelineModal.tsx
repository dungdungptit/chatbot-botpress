  ...
  return (
    ...
    >
      <div className={Classes.DIALOG_BODY}>
        <RadioGroup
          label="Bạn muốn sử dụng quy trình nào?"
        ...
        >
          ...
          {custom && <Radio id="radio-bp-custom" label={`Tùy chỉnh  [${custom.join(', ')}]`} value="custom" />}
        </RadioGroup>
        <br />
        <Checkbox
          checked={resetStage}
          onChange={e => setResetStage(e.currentTarget.checked)}
          label="Di chuyển tất cả các bot đến giai đoạn đầu tiên của quy trình"
        />
      </div>

      <div className={Classes.DIALOG_FOOTER}>
        <div className={Classes.DIALOG_FOOTER_ACTIONS}>
          <Button id="btn-submit-change-pipeline" text="Lưu" onClick={submit} />
        </div>
      </div>
    </Dialog>
  )
