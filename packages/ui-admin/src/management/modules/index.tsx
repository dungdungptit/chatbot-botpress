...
const Modules: FC<Props> = props => {
  ...
  const restartServer = async () => {
    try {
      if (
        await confirmDialog(
          'Bạn có chắc không? Nếu bạn có nhiều máy chủ, tất cả chúng sẽ được khởi động lại cùng một lúc.',
          { acceptLabel: 'Khởi động lại máy hủ ngay bây giờ' }
        )
      ) {
  ...
  const updateModuleStatus = async (moduleName: string, enabled: boolean) => {
    try {
      ...
      props.fetchModules()
      toast.success('Trạng thái mô-đun được cập nhật thành công')
    } catch (err) {
      toast.failure(err.message)
    }
  }
  ...
  const unpackModule = async (moduleName: string) => {
    try {
      ...
      toast.success('Giải nén mô-đun thành công')
      props.fetchModules()
    } catch (err) {
      toast.failure(err.message)
    }
