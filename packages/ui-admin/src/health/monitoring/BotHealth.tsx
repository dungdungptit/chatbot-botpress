...
const STATUS: Option[] = [
  ...
  { label: 'Không bình thường', value: 'unhealthy' },
  { label: 'Bị vô hiệu hóa', value: 'disabled' }
]
...
const BotHealth: FC<Props> = props => {
            ...
            Cell: cell => {
              switch (_.get(cell.original, `data[${key}].status`)) {
                default:
                  return 'N/A'
                case 'unhealthy':
                  return <span className={style.logCritical}>Không bình thường</span>
                case 'healthy':
                  return <span className={style.logInfo}>Bình thường</span>
                case 'disabled':
                  return 'Disabled'
              }
            },
    ...
    setColumns([
      {
        ...
        Cell: x => {
          return (
            <span>
              <Tooltip hoverOpenDelay={1000} content="Xem logs cho bot này">
  ...
  const reloadBot = async (botId: string) => {
    if (!(await confirmDialog(`Bạn có chắc chắn muốn tải lại bot ${botId}?`, { acceptLabel: 'Tải lại bot' }))) {
      return
    }

    try {
      await api.getSecured().post(`/admin/workspace/bots/${botId}/reload`)
      toast.success('Kết nối lại tới bot thành công')
    } catch (err) {
      console.error(err)
      toast.failure('Không kết nối tới bot. Kiểm tra nhật ký máy chủ để biết chi tiết')
    }
  }
